import { useState } from 'react';
import RegistrationForm from './components/RegistrationForm';
import FormikForm from './components/FormikForm';
import './App.css';

function App() {
  // State to track current form view
  const [currentForm, setCurrentForm] = useState('controlled');

  return (
    <div className="App">
      <div style={{ textAlign: 'center', marginBottom: '20px' }}>
        <h1>React Form Management</h1>
        <div style={{ marginBottom: '20px' }}>
          {/* Button to switch to controlled components form */}
          <button 
            onClick={() => setCurrentForm('controlled')}
            style={{
              margin: '0 10px',
              padding: '10px 20px',
              backgroundColor: currentForm === 'controlled' ? '#007bff' : '#f8f9fa',
              color: currentForm === 'controlled' ? 'white' : 'black',
              border: '1px solid #007bff',
              borderRadius: '4px',
              cursor: 'pointer'
            }}
          >
            Controlled Components
          </button>
          {/* Button to switch to Formik form */}
          <button 
            onClick={() => setCurrentForm('formik')}
            style={{
              margin: '0 10px',
              padding: '10px 20px',
              backgroundColor: currentForm === 'formik' ? '#28a745' : '#f8f9fa',
              color: currentForm === 'formik' ? 'white' : 'black',
              border: '1px solid #28a745',
              borderRadius: '4px',
              cursor: 'pointer'
            }}
          >
            Formik
          </button>
        </div>
      </div>

      {/* Render the selected form component */}
      {currentForm === 'controlled' ? <RegistrationForm /> : <FormikForm />}
    </div>
  );
}

export default App;